...
export default class Map1Scene extends Phaser.Scene {
  ...
  init(data: { avatar: string; socket: any; socketId: string }) {
  ...
  private createOtherPlayer(id: string, data: PlayerData, avatarKey: string) {
    ...
    const anim = `${avatarKey}-idle-${data.direction || "down"}`;
    otherPlayer.anims.play(anim, true);
